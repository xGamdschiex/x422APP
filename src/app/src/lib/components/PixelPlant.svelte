<script lang="ts">
  ...
  // Stage-Nummer aus Phase + Woche/Tag berechnen
  function getStageNum(p: string, w: number, t: number): number {
    const pl = p.toLowerCase();
    if (pl === 'keimung') {
      return Math.min(t, 4); // 01–04
    } else if (pl === 'veg') {
      return Math.min(4 + w, 10); // 05–10
    } else if (pl === 'bluete' || pl === 'bloom') {
      return Math.min(10 + w, 20); // 11–20
    } else if (pl === 'trocknung' || pl === 'flush') {
      return Math.min(20 + w, 23); // 21–23
    } else if (pl === 'curing') {
      return Math.min(23 + w, 25); // 24–25
    }
    return 1;
  }

  $: stageNum = getStageNum(phase, woche, tag);
  $: imgSrc = `/sprites/plants/plant${String(stageNum).padStart(2, '0')}.png`;

  let imgFailed = false;
  $: { imgSrc; imgFailed = false; } // Reset bei Stage-Wechsel

  // ── SVG-Fallback (original Low-Poly) ──────────────────────────────────────
  ...
  function cannabisLeaf(cx: number, cy: number, dir: number, len: number, fc: number, color: string, colorDark: string): string {
    ...
    if (fc >= 7) {
      fingerDefs.push([-90, 1.0], [-70, 0.85], [-110, 0.85], [-50, 0.65], [-130, 0.65], [-35, 0.4], [-145, 0.4]);
    } else if (fc >= 5) {
      fingerDefs.push([-90, 1.0], [-65, 0.8], [-115, 0.8], [-40, 0.5], [-140, 0.5]);
    } else {
      fingerDefs.push([-90, 1.0], [-55, 0.65], [-125, 0.65]);
    }
...
<div class="plant-wrap flex justify-center">
  {#if !imgFailed}
    <img
      src={imgSrc}
      alt="Pflanze Stage {stageNum}"
      width="210"
      height="210"
      class="plant-img"
    ...
    />
  {:else}
    <!-- SVG-Fallback wenn PNG nicht vorhanden -->
    <svg viewBox="0 0 96 96" width="210" height="210" xmlns="http://www.w3.org/2000/svg" class="plant-svg">
      ...
      {:else}
        ...
        {#if hasTrichomes}
          {#each Array.from({ length: Math.min(woche * 4, 24) }, (_, ti) => ti) as ti}
            ...
            <circle cx={tx} cy={ty - 1.4} r="0.5" fill={tColor} opacity={0.5 + (ti % 3) * 0.15} />
          {/each}
        {/if}
        {#if woche >= 3}
